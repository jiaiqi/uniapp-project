		...
		<view class="parent-node cu-item text-green animation-slide-top">
			<view class="parent-list" :class="{ 'single-parent': singleNode }">
				<view class="list-item" v-for="(item, index) in treeData" :key="index" @click="clickParentNode(item, index)">
					...
					<view class="label" :class="{ active: item[nodeKey] === activeNode[nodeKey], normal: item[nodeKey] !== activeNode[nodeKey] }">
						{{buildShowText(item) }}
					</view>
				</view>
			</view>
...
export default {
				...
				if (this.treeData.length === 0) {
					...
					return this.treeData.filter(
						tree =>
							...
							(tree._childNode && Array.isArray(tree._childNode) && tree._childNode.length === 0 && tree.parent_no !== null)
					).length > 0
						? false
						: true;
				}
	...
	methods: {
		buildShowText(item){
			...
			}else if(this.srvInfo.key_disp_col && this.srvInfo.refed_col){
				result = item[this.srvInfo.key_disp_col] + '/' + item[this.srvInfo.refed_col]
			}
			return result
			// item[disColName] ? item[disColName] : srvInfo.key_disp_col && srvInfo.refed_col ?
			// (item[srvInfo.key_disp_col] + '/' + srvInfo.refed_col) : ''
		},
		onTree(e) {
