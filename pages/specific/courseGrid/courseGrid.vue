		...
		methods: {
			...
			goPage(e) {
				...
				if (scheduleConfig.page_type && scheduleConfig.page_type === 'proc') {
					uni.navigateTo({
						url: `/pages/public/list/list?cond=${decodeURIComponent(JSON.stringify(cond))}&serviceName=${	scheduleConfig.srv}&pageType=proc&viewTemp=${decodeURIComponent(JSON.stringify(viewTemp))}&destApp=${scheduleConfig.app}`
					});
				} else {
					uni.navigateTo({
						url: `/pages/public/list/list?cond=${decodeURIComponent(JSON.stringify(cond))}&serviceName=${	scheduleConfig.srv}&pageType=list&viewTemp=${decodeURIComponent(JSON.stringify(viewTemp))}&destApp=${scheduleConfig.app}`
					});
				}
			},
				...
				if (res.data.state === 'SUCCESS' && res.data.data.length > 0) {
					...
					if (this.scheduleConfig?.row_head_type === '排程结果字段' && this.scheduleConfig?.row_head_sc_col) {
						if (Array.isArray(scheduleItemList) && scheduleItemList.length > 0) {
							...
							// const data = this.deepClone(scheduleItemList[0].data)
							const data = this.deepClone(maxLengthData)
							// if (Array.isArray(data) && data.find(item => item[this.scheduleConfig.row_head_sc_col])) {
							rowHeadList = data.map(item => {
