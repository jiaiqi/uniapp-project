	...
	<div class="symptom-select">
		...
		<view class="menu-wrap" v-if="symptomList && symptomList.length > 0">
			...
			<view class="menu-right">
				<view class="symptom-box" v-for="(item, index) in symptomList[currentTab].children" :key="index">
					<view class="title" v-if="item && Array.isArray(item.children) && item.children.length > 0">{{ item.name }}</view>
					<view class="symptom-item" v-for="(sym, index) in item.children" :key="sym.no">
						<view class="symptom-name" :class="{ checked: sym.checked }">{{ sym.name }}</view>
						<view class="check-box"><u-switch v-model="sym.checked"></u-switch></view>
					</view>
				</view>
			</view>
		</view>
...
export default {
	...
	methods: {
		submit() {
			...
			uni.setStorageSync('symptomList', resultList);
	...
	created() {
		...
		this.getSymptomList(symptomList);
	}
};
</script>
