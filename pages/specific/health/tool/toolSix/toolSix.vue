	...
	<view class="wrap">
		...
		<view class="" v-if="isNormalStyle"> 
			...
			<view class=" illustration">
				<image src="/static/img/sport.jpg" mode=""></image>
			</view>
			<view class="sportbox">
...
<script>
	export default{
		data(){
			return{
				icon:require('@/static/icon-burn.png'),
				nutrient:[
			...
			async selectDataOrgin(name){
				...
				if(dataItem.length > 0){
					this.getFilePath( dataItem[0].image).then(business_img=>{
						...
						// uni.navigateTo({
						// 	url:'/pages/specific/health/sportsDetail/sportsDetail?sport=' + encodeURIComponent(JSON.stringify(dataItem[0]))
						// })
					})
				}
