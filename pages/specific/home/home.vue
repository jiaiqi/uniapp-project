							...
							<view class="cu-item" v-for="(item, index) in listItem" :key="index">
								<view class="content padding-tb-sm">
									...
									<view class="titleWrap" @click.stop="toPreview(item.activity_no, item.status, item.fill_batch_no)">
										...
										<view class="cu-tag round " v-if="currentTab === 0" :class="item.status == '未开始' ? 'bg-orange' : item.status == '已结束' ? 'bg-grey' : 'bg-olive'">
											{{ item.status }}
										</view>
										<view class="cu-tag round " v-if="currentTab === 1" :class="item.state == '未完成' ? 'bg-orange' : item.status == '完成' ? 'bg-grey' : 'bg-olive'">
											{{ item.state }}
										</view>
									</view>
									...
									<view class="action line_action">
										...
										<view v-if="item.status === '未开始'" @click.stop="updateState(item)" class="btn release">发布</view>
										<view v-if="item.status === '未开始' || item.modify_after_submit === '是'" @click.stop="toUpdate(item.activity_no)" class="btn update_btn">修改</view>
...
export default {
	...
	methods: {
		...
		async getList(e) {
			let that = this;
			if (e === 0) {
				let url = this.getServiceUrl(this.appName ? this.appName : 'daq', 'srvdaq_activity_mine_select', 'select');
				let req = {
				...
				let url = this.getServiceUrl(this.appName ? this.appName : 'daq', 'srvdaq_activity_mine_feedback_select', 'select');
				let req = {
					serviceName: 'srvdaq_activity_mine_feedback_select',
					colNames: ['*'],
					order: [{ colName: 'create_time', orderType: 'desc' }],
					page: { pageNo: this.pageInfo.pageNo, rownumber: this.pageInfo.rownumber }
				};
				...
				if (response.data.state === 'SUCCESS' && response.data.data.length > 0) {
					...
					for (let item of response.data.data) {
						let list = await this.getFeedbackList(item);
						if (Array.isArray(list)) {
							questList = questList.concat(list);
		...
		async getFeedbackList(item) {
			// 反馈列表
			let url = this.getServiceUrl(this.appName ? this.appName : 'daq', 'srvdaq_record_reply_select', 'select');
			let req = {
				...
				condition: [
					...
					},
					{
						colName: 'user_no',
						ruleType: 'in',
						value: this.currentLoginUser
					}
				],
				order: [{ colName: 'create_time', orderType: 'desc' }]
			};
			let response2 = await this.$http.post(url, req);
